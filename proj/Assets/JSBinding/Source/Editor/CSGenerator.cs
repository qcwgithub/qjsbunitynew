        ...
        for (int i = 0; i < fields.Length; i++)
        {
            ...
            if (!bReadOnly)
            {
                if (!isDelegate)
                {
                    sb.Append("else\n");
                    if (field.IsStatic)
                        sb.AppendFormat("{0}.{1} = ({2}){3};\n", type.Name, field.Name, GetTypeFullName(field.FieldType), BuildRetriveParam(field.FieldType));
                    else
                    {
                        if (type.IsValueType)
                        {
...
// 
    ...
    [MenuItem("Assets/JSBinding/Generate JS and CS Bindings")]
    ...
    [MenuItem("Assets/JSBinding/Output All Types in UnityEngine")]
